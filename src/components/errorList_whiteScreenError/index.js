...
const ErrorList_whiteScreenError = (props) => {
	const columns = [
		{
			title: "Time",
			dataIndex: "time",
	...
	const getRes = async () => {
		const { data } = await errorList(urlPram, "whiteScreenError");
	...
	return (
		<Row gutter={[16, 16]}>
			<Col span={24}>
				<Graph url='localhost' dataType='whiteScreenError' />
			</Col>
			<Col span={24}>
				<Card title='whiteScreen错误数目统计列表' bordered={false}>
...
export { ErrorList_whiteScreenError };
